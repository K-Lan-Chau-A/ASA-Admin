...
export function EditPackageDialog({ packageData, open, onOpenChange, onPackageUpdated }: EditPackageDialogProps) {
  ...
  // Populate form when package data changes
  useEffect(() => {
    if (packageData) {
      setFormData({
        ...
        requestLimit: String(packageData.requestLimit || 0),
        accountLimit: String(packageData.accountLimit || 0),
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        ...
        createdAt: packageData.features?.[0]?.featureId ? undefined : new Date().toISOString(),
        updatedAt: new Date().toISOString()
      }

      // Remove createdAt if it doesn't exist in original data (for editing)
      if (!packageData.features?.[0]?.featureId) {
        delete (payload as any).createdAt
      }

      const response = await fetch(`${API_URL}/api/products/${packageData.productId}`, {
        method: 'PUT',
        headers: {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[600px] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            {pt.editPackageDescription} {packageData.productName}
          </DialogDescription>
        </DialogHeader>
